...
```{r}
data <- read.csv("/Users/theophilasetiawan/Downloads/2017-2023-10-Checkouts-SPL-Data.csv")
```
...
```{r}
summary <- source("/Users/theophilasetiawan/Documents/INFO 201 (main)/Info201code/a3-spl-checkouts-theophilaabigail/summary.R")
```
The most checked out material type is BOOK, which surprised me a little because I thought it would be EBOOK. The most checked out book is SPL HotSpot : connecting Seattle / [distributed by T-Mobile], and I personally have never heard of that book. I wanted to know if the top sales book would be a book I know or one of my favorite, but it was not. The year in which the most checked out material type is books is 2018, which is to be expected since in 2018 digital books were not too popular yet. The year in which the most usage class checked out is digital is 2022, which makes sense because more people are going digital these days. The top 3 publishers based on the most number of check outs are Random House, Inc. , Books on Tape , and HarperCollins Publishers Inc., and I have only heard of HarperCollins.
...
```{r}
chart2 <- source("/Users/theophilasetiawan/Documents/INFO 201 (main)/Info201code/a3-spl-checkouts-theophilaabigail/Chart_2_top_publishers.R")
